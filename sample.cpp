void NSEStreamingData::frameQuoteMPB(struct interactiveMbpData *tInterMbpData)
{
	...
	iQuoteCache->second.dTotalTradeValue = 
		((iQuoteCache->second.dAvgPrice * iQuoteCache->second.lTotalVolume) / 10000000);
	...
	if (segment->marketOpen) {
		...
		if (iQuoteCache->second.dClosePrice > 0) {
			...
			iQuoteCache->second.dPClosePrice =
				iQuoteCache->second.dClosePrice;
	...
	for (uint8_t i = 0; i < 10; i++)
	{
		if (i < 5)
		{
			...
			iQuoteCache->second.bid_entry[i % 5].dPrice = iQuoteCache->second.bid_entry[i % 5].dPrice;
		...
		else
		{
			...
			iQuoteCache->second.ask_entry[i % 5].dPrice = iQuoteCache->second.ask_entry[i % 5].dPrice;
